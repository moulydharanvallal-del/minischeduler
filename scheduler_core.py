...
def infer_raw_materials(bom_data):
    """Infer raw materials from bom_data.

    In this notebook-derived schema:
      - Each BOM row has an output part in `part_name`
      - Each row lists consumed inputs in `inputs_needed` as a comma-separated string
    A raw material is any input that is never produced as a `part_name`.
    """
    produced = set()
    consumed = set()
    ...
    for row in bom_data:
        if not isinstance(row, dict):
            continue
        out = row.get("part_name")
        if out:
            produced.add(str(out).strip())

        ins = row.get("inputs_needed") or ""
        # inputs_needed may be a comma-separated string
        if isinstance(ins, str):
            parts = [p.strip() for p in ins.split(",") if p.strip()]
        elif isinstance(ins, (list, tuple, set)):
            parts = [str(p).strip() for p in ins if str(p).strip()]
        else:
            parts = []
        for p in parts:
            consumed.add(p)
...
def normalize_raw_materials(raw_materials):
    """Normalize raw materials into list[dict] with at least {'part': <str>}."""
    ...
    if isinstance(raw_materials, (list, tuple)):
        ...
        for x in raw_materials:
            if isinstance(x, str):
                out.append({"part": x})
            elif isinstance(x, dict) and "part" in x:
                out.append(dict(x))
            else:
                raise ValueError("raw_materials must be a list of strings or dicts containing a 'part' key.")
        return out
    raise ValueError("raw_materials must be a list of strings or dicts containing a 'part' key.")

def validate_raw_materials(bom_data, raw_materials, *, strict_missing=True):
    """Validate declared raw materials against BOM inference."""
    inferred = set(infer_raw_materials(bom_data))
    declared = normalize_raw_materials(raw_materials)
    declared_parts = {d["part"] for d in declared}

    missing = sorted(inferred - declared_parts)

    if missing and strict_missing:
        raise ValueError(
            "Raw materials missing from Raw Materials list (but required by BOM): "
            + ", ".join(missing)
        )

    for p in missing:
        declared.append({"part": p})

    return declared

# Default raw materials inferred from bom_data
DEFAULT_RAW_MATERIALS = [{"part": p} for p in infer_raw_materials(bom_data)]

def run_scheduler(bom_data, customer_orders, work_center_capacity, raw_materials=None, *, base_start=None, show_chart=True, strict_raw_materials=False):
    """Runs planning + scheduling and returns (scheduled_runs, work_orders, plan, fig)."""
    raw_materials_norm = validate_raw_materials(
        bom_data,
        raw_materials if raw_materials is not None else DEFAULT_RAW_MATERIALS,
        strict_missing=strict_raw_materials
    )

    bom_index, last_step_by_part = build_bom_index(bom_data)
    plan = plan_with_step_inventory(customer_orders, bom_index, last_step_by_part)

    plan["raw_materials"] = raw_materials_norm
    plan["raw_materials_inferred"] = infer_raw_materials(bom_data)
